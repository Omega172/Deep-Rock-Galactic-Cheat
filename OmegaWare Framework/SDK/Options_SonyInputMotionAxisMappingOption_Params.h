#pragma once

/**
 * Name: Deep_Rock_Galactic
 * Version: 1.38.94396.0
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function Options_SonyInputMotionAxisMappingOption.Options_SonyInputMotionAxisMappingOption_C.Construct
	 */
	struct UOptions_SonyInputMotionAxisMappingOption_C_Construct_Params
	{	};

	/**
	 * Function Options_SonyInputMotionAxisMappingOption.Options_SonyInputMotionAxisMappingOption_C.UINeedsUpdate
	 */
	struct UOptions_SonyInputMotionAxisMappingOption_C_UINeedsUpdate_Params
	{	};

	/**
	 * Function Options_SonyInputMotionAxisMappingOption.Options_SonyInputMotionAxisMappingOption_C.ShowUI
	 */
	struct UOptions_SonyInputMotionAxisMappingOption_C_ShowUI_Params
	{	};

	/**
	 * Function Options_SonyInputMotionAxisMappingOption.Options_SonyInputMotionAxisMappingOption_C.BndEvt__Basic_OptionSwitcher_K2Node_ComponentBoundEvent_20_OnSelectionChanged__DelegateSignature
	 */
	struct UOptions_SonyInputMotionAxisMappingOption_C_BndEvt__Basic_OptionSwitcher_K2Node_ComponentBoundEvent_20_OnSelectionChanged__DelegateSignature_Params
	{
	public:
		class FText                                                Value;                                                   // 0x0000(0x0018)  (BlueprintVisible, BlueprintReadOnly, Parm)
		int32_t                                                    Index;                                                   // 0x0018(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function Options_SonyInputMotionAxisMappingOption.Options_SonyInputMotionAxisMappingOption_C.ExecuteUbergraph_Options_SonyInputMotionAxisMappingOption
	 */
	struct UOptions_SonyInputMotionAxisMappingOption_C_ExecuteUbergraph_Options_SonyInputMotionAxisMappingOption_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
