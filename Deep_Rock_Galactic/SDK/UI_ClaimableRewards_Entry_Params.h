#pragma once

/**
 * Name: Deep_Rock_Galactic
 * Version: 1.38.91584.0
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.SequenceEvent__ENTRYPOINTUI_ClaimableRewards_Entry_2
	 */
	struct UUI_ClaimableRewards_Entry_C_SequenceEvent__ENTRYPOINTUI_ClaimableRewards_Entry_2_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.SequenceEvent__ENTRYPOINTUI_ClaimableRewards_Entry_1
	 */
	struct UUI_ClaimableRewards_Entry_C_SequenceEvent__ENTRYPOINTUI_ClaimableRewards_Entry_1_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.Refresh
	 */
	struct UUI_ClaimableRewards_Entry_C_Refresh_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.PlayRevealAudio
	 */
	struct UUI_ClaimableRewards_Entry_C_PlayRevealAudio_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.PreConstruct
	 */
	struct UUI_ClaimableRewards_Entry_C_PreConstruct_Params
	{
	public:
		bool                                                       IsDesignTime;                                            // 0x0000(0x0001)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.Construct
	 */
	struct UUI_ClaimableRewards_Entry_C_Construct_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.OnClaimAnimFinished
	 */
	struct UUI_ClaimableRewards_Entry_C_OnClaimAnimFinished_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.BndEvt__ITM_BigButton_K2Node_ComponentBoundEvent_1_OnClicked__DelegateSignature
	 */
	struct UUI_ClaimableRewards_Entry_C_BndEvt__ITM_BigButton_K2Node_ComponentBoundEvent_1_OnClicked__DelegateSignature_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.OnPopupImageLoaded
	 */
	struct UUI_ClaimableRewards_Entry_C_OnPopupImageLoaded_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.PlaySlamAudio
	 */
	struct UUI_ClaimableRewards_Entry_C_PlaySlamAudio_Params
	{	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.ReceiveMoveIn
	 */
	struct UUI_ClaimableRewards_Entry_C_ReceiveMoveIn_Params
	{
	public:
		float                                                      InStartDelay;                                            // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

	/**
	 * Function UI_ClaimableRewards_Entry.UI_ClaimableRewards_Entry_C.ExecuteUbergraph_UI_ClaimableRewards_Entry
	 */
	struct UUI_ClaimableRewards_Entry_C_ExecuteUbergraph_UI_ClaimableRewards_Entry_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
